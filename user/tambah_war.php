...
<!-- Nav Item - User Information -->
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Tambah Warmerking</h1>
                    </div>
                ...
                <div class="form-group">
                    <label class="col-md-12 mb-0">Deskripsi</label>
                        <div class="col-md-12">
                            <textarea rows="5" name="deskripsi_dok" class="form-control ps-0 form-control-line"></textarea>
                        </div>
                </div>
    <div class="col-md-10">
 ...
 session_start();
        ...
        if (!empty($_FILES["myFile"])) {
            ...
            $deskripsi_dok  = $_POST['deskripsi_dok'];
            $penerbit_dok   = $_POST['penerbit_dok'];
          ...
          if ($myFile["error"] !== UPLOAD_ERR_OK) {
            echo "<script>alert('Data Gagal Disimpan, COBA LAGI');window.location='tambah_war.php'</script>";
            exit;
          }
          ...
          $success = move_uploaded_file($myFile["tmp_name"],
            UPLOAD_DIR . $name);
          if (!$success) { 
            echo "<script>alert('Data Gagal Disimpan, COBA LAGI');window.location='warmerking.php'</script>";
            exit;
          }else{

    
            $insert = $conn->query("INSERT INTO warmerking(id_war, username, nm_pemohon, nik, jenis_dok, penerbit_dok, deskripsi_dok, tgl_upload, file_name, file_size, file_type, status)
            VALUES('$id_war','$username', '$nm_pemohon' ,'$nik', '$jenis_dok', '$penerbit_dok', '$deskripsi_dok', '$tgl', '$name', '$size', '$ext', '$status')");

            // $insert = $conn->query("INSERT INTO warmerking(id_war, username, nm_pemohon, nik, jenis_dok, penerbit_dok, tgl_upload, file_name, file_size, file_type)
            // VALUES('$kodeauto','$username','$nm_pemohon', '$nik', '$jenis_dok', '$penerbit_dok', '$tgl', '$name', '$size', '$ext')");
            if($insert){
              echo "<script>alert('Data Sukses Disimpan');window.location='warmerking.php'</script>";
            }else{
              echo "<script>alert('Data Gagal Disimpan, COBA LAGI');window.location='tambah_war.php'</script>";
              exit;
            }
