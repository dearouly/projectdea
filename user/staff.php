...
<br>
		...
		$no = 1;
        // WHERE level = 'NOT01';"
		$data = mysqli_query($koneksi,"select * from user WHERE level= 'staff'; ");
		while($d = mysqli_fetch_array($data)){
			?>
			<tr>
				<td><?php echo $no++; ?></td>
                <td>USR<?php echo $d['id']; ?></td>
				<td><?php echo $d['nama']; ?></td>
				<td><?php echo $d['email']; ?></td>
				<td><?php echo $d['username']; ?></td>
                <td><?php echo $d['tgl_daftar']; ?></td>
				<td>
                <a href="edit_staff.php?id=<?php echo $d['id']; ?>"class="btn btn-success btn-sm">EDIT</a>
				<a href="hapus_staff.php?id=<?php echo $d['id']; ?>" onclick= "return confirm('Yakin ingin menghapus data ini?');" class="btn btn-danger btn-sm">HAPUS</a>
				</td>
			</tr>
			<?php 
		}
